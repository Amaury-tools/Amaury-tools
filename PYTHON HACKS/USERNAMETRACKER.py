    ...
    for site, url_template in sites.items():
        try:
            ...
            try:
                ...
                if found == True:
                    print(f"{GREEN}Found: {site} -> {WHITE + url}\n{RESET}")
                elif found == False:
                    print(f"{RED}Not Found: {site} -> {WHITE + url}\n{RESET}")

            except Exception as e:
                print(f"{RED}Error: {site}: {WHITE + str(e)}\n{RESET}")
        except: 
            pass

    print(f"\n{CYAN}Total Websites Checked: {WHITE}{number_site}{RESET}")
    print(f"{CYAN}Total Found: {WHITE}{number_found}{RESET}")
    for site_and_url_found in sites_and_urls_found:
        time.sleep(0.5)
        print(f"{CYAN}{site_and_url_found}{RESET}")
